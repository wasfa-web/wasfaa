...
// ---------------------------------------------
// دالة لإضافة الإيموجي (للاقتراحات فقط)
// ---------------------------------------------
...
function addRecipe() {
  ...
  let ingredients = recipeIngredients.value
    .split(/[,،\s]+| و /) // فصل على فاصلة، فراغ، أو حرف الواو
    .map(i => i.trim())
    .filter(i => i);
 ...
 if (imageFile) {
  compressImage(imageFile, image => saveRecipe(image));
} else {
  saveRecipe("");
}
...
// ---------------------------------------------
// الاقتراح التلقائي عند الكتابة (مع الإيموجي فقط)
// ---------------------------------------------
